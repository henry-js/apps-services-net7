                ...
                page.Content()
                    ...
                    .Table(table =>
                    {
                        ...
                        foreach (var item in Model.Categories)
                        {
                            ...
                            string imagePath = Path.Combine(CurrentDirectory, "images", $"category{item.CategoryId}.jpeg");

                            table.Cell().Image(imagePath);
                        }
                    });
                page.Footer()
                    .Height(50).Background(color: Colors.Grey.Lighten1)
                    .AlignCenter().Text(content: x =>
                    {
                        x.CurrentPageNumber();
                        x.Span(" of ");
                        x.TotalPages();
